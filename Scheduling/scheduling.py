...
def schedule_day(day, task_list):
    ...
    free_time = day.free_time # List of available time slots in order of occurence
    while len(free_time) > 0 and len(todos) > 0: # While there's stuff to do and time to do it...
        time_slot = free_time[0]
        ...
        if len(fit_tasks) == 0: # If no tasks fit in this block, delete it from the queue and skip it
            del free_time[0]
            continue
        time_area = time_slot[0]//480 # What part of the day it is, to determine user's work time preferences
        ...
        if fit_tasks[0].duration + min_event_time >= avail_time: # If a task will consume a time block, delete it entirely
            ...
            free_time[0] = (fit_tasks[0].end, time_slot[1]) # If there will be some time left over, make that the next time block
...
def add_item(name, duration, importance, category):
    ...
    new_item = Item(name, duration = duration, importance = importance, category = category)
    todos.append(new_item)

date = '08/09/2004'
calendar = Calendar('My Calendar')
today = Day(date) # In practice, these would be initialized and stored in the calendar long ahead of time
calendar.days[date] = today

busy_time = random_timeblocks(8) # In practice, this would come from the events scheduled today
print(busy_time)
today.free_time = busy_to_free(busy_time)
print(today.free_time)

min_event_time = 20 # This would be a user preference
